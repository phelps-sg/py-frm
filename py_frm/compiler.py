    ...
    def extract_info(node):
        if isinstance(node, ast.FunctionDef):
            for statement in node.body:
                if isinstance(statement, ast.Return) and isinstance(statement.value, ast.GeneratorExp):
        ...
        for gen in gen_exp.generators:
            for if_clause in gen.ifs:
                if isinstance(if_clause, ast.Compare) and isinstance(if_clause.ops[0], ast.Eq):
                    ...
                    right_var, right_attr = if_clause.comparators[0].value.id, if_clause.comparators[0].attr
                    left_model = var_to_model_map[left_var]
                    right_model = var_to_model_map[right_var]
                    conditions.append(getattr(left_model, left_attr) == getattr(right_model, right_attr))
